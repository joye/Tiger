...
namespace object
{
  ...
  {
    // FIXME: Some code was deleted here (Check int, then string, then class name). (Optional)
  }
                                            ...
                                            const type::Method* dispatch_method)
  {
    ...
    for (const type::Class* c : class_type->subclasses_get())
      if (ranges::any_of(*typechunk, [c](const ast::TypeDec* tmp) {
          ...
          for (auto a = c->attrs_get().begin(); a != c->attrs_get().end(); a++)
            {
              ...
              // FIXME: Some code was deleted here (Initialize attr). (Optional)
                                      ...
                                      misc::symbol class_name)
  {
    ...
    // Get the other arguments.
    const ast::MethodDec* def;
    // FIXME: Some code was deleted here (Initiliaze def). (Optional)
    ...
    for (const type::Class* c = cls->super_get(); c; c = c->super_get())
      {
        ...
        if (ranges::any_of(e, [c](const ast::TypeDec* t) {
        ...
        // Get the other arguments.
        const ast::MethodDec* def;
        // FIXME: Some code was deleted here (Initialize def). (Optional)
        for (const ast::VarDec* arg : def->formals_get())
          functions << ", " << arg->name_get() << " : "
                    << recurse(*arg->type_name_get());
        functions << ")";
    ...
    if (cls->has_data())
      {
        types_ << " type " << class_contents_prefix << class_name << " ="
               << " { ";
        // FIXME: Some code was deleted here (Populate the record with attributes (names and types)). (Optional)
        types_ << " }\n";
      }
    ...
    for (const ast::TypeDec* t : e)
      {
        ...
        // FIXME: Some code was deleted here (Get the ty's class type). (Optional)
  ...
  void DesugarVisitor::operator()(const ast::AssignExp& e)
  {
    ...
    ast::Var* var = nullptr;
    // FIXME: Some code was deleted here (Recurse). (Optional)
  ...
  void DesugarVisitor::operator()(const ast::CallExp& e)
  {
    ...
    const ast::Location& location = e.location_get();
    // FIXME: Some code was deleted here (Grab name). (Optional)

    // FIXME: Some code was deleted here (Actual arguments). (Optional)
    ...
    // FIXME: Some code was deleted here (Instantiate into result). (Optional)
  }
  ...
  void DesugarVisitor::operator()(const ast::MethodCallExp& e)
  {
    ...
    // FIXME: Some code was deleted here (Adapt type). (Optional)
    ...
    // FIXME: Some code was deleted here (Instanciate into result). (Optional)
  }
  ...
  {
    bool is_main;
    // FIXME: Some code was deleted here (Setup is_main). (Optional)
    if (is_main)
      {
